...
### ② AMapManager (오브젝트 풀링 & **그리드 데이터 중앙 매니저**)

- **역할:** 화면에 보이는 만큼의 블록만 메모리에 유지(풀링)하며, **[핵심] 2차원 논리적 장부(GridMap)를 통해 모든 블록의 위치 데이터를 중앙 통제하고, 빈 공간 발생 시 오차 없는 중력(낙하) 연산을 지시하는 MVC 패턴의 컨트롤러(Controller) 역할을 수행합니다.**
- **주요 변수:**
    - `TMap<FIntPoint, ABlock*> GridMap`: **[신규]** 논리적 2차원 바둑판 데이터 장부. (블록의 물리적 위치가 아닌 논리적 좌표 관리)
    - `TArray<ABlock*> BlockPool`: 재활용 블록 배열 (화면 높이 + 여유분)
    - `float TileSize`: 타일의 기준 크기
- **주요 함수:**
    - `InitializeMap()`: 초기 풀 스폰 및 그리드 장부(`RegisterBlock`) 초기화.
    - `RegisterBlock() / RemoveBlockFromGrid()`: **[신규]** 장부에 블록의 데이터를 등록/삭제하는 헬퍼 함수.
    - `ProcessFalling(Col, StartRow)`: **[신규]** 특정 좌표 블록 파괴 시 윗줄을 역순으로 검사하여, 빈 공간만큼 블록의 데이터 좌표를 먼저 갱신하고 시각적 낙하(`PrepareToFall`)를 지시하는 중앙 중력 로직.
    - `RecycleTopLine()` / `OnLevelUpExplosion()` / `ReturnBlockToPool()`

### ③ ABlock (부모 클래스 - 다형성 및 **시각적 뷰**)

- **역할:** 모든 채굴 대상 및 상호작용 오브젝트의 기본형. **물리 엔진(Raycast) 의존성을 버리고, 맵 매니저의 지시를 받아 시각적 이동(View) 및 애니메이션 처리에 집중하는 수동적 객체로 설계되었습니다.**
- **주요 변수:**
    - `int32 GridX, GridY`: **[신규]** 바둑판 내 자신의 현재 논리적 좌표 (사원증).
    - `AMapManager* mapManagerRef`: **[신규]** 무거운 `Cast` 연산을 피하기 위해 스폰 시 주입받는 매니저 캐싱 포인터.
- **상태 정의:** `EBlockState` (Idle, Anticipating, Falling)
- **주요 함수:**
    - `PrepareToFall(TargetLocation)` & `ActuallyStartFalling()`: **[신규]** 매니저의 중력 지시를 받아 덜덜 흔들리는 대기(Anticipating) 후, 목표 위치로 부드럽게 추락하는 상태 머신 동작.
    - `ExecuteFloodFill(...)`:  재귀 함수. 물리적 레이캐스트 대신, 매니저의 장부(`GetBlockAtGrid`)를 조회하여 상/하/좌/우 인접한 동일 색상 블록을 초고속으로 탐색.
    - `Pop()`:  파괴 시 장부에서 자신을 지우고, 매니저에게 중력 연산(`ProcessFalling`)을 요청한 뒤 파괴 이펙트 재생 및 충돌 해제.
    - `TryKillPlayer(AActor* TargetActor)` / `OnInteracted()` / `ResetBlock()`
...
- **연쇄 파괴 및 중력 시스템 (Grid-Based Chain Reaction):**
    - 기존 물리 엔진의 레이캐스트(Line Trace) 방식에서 논리적 TMap 데이터 장부 기반(Grid-based)으로 아키텍처를 전면 개편하여 물리 오차(프레임 드랍, 겹침 현상)를 원천 차단함.
    - **중력 통제:** 블록 파괴 시 매니저가 즉시 데이터 장부를 갱신하여 윗블록들의 새로운 목표 좌표를 선점하고 시각적 낙하를 예약함.
    - **매치 4 & 플러드 필(Flood Fill):** 블록이 낙하를 마치고 정착(Idle)하는 순간, 매니저의 데이터 장부를 기준으로 상/하/좌/우 4방향을 재귀 탐색하여 동일한 색상(타입)의 블록을 찾음.
    - 동일 색상 블록이 4개 이상 연결되어 있을 경우 일괄 파괴(`Pop`)되며, 빈 공간이 발생함에 따라 다시 중앙 통제식 중력 연쇄 낙하가 발생하는 무결점 콤보(Chain Reaction) 시스템 적용.
